...
const PriceChecker = () => {
  ...
  const [values, setValues] = useState({ linkType: 1 });
  ...
  useEffect(() => {
    const getElements = async () => {
      try {
        const res = await axiosPrivate.get("/api/Link?type=1");
    ...
    if (isValidTitle && isValidUrl) {
      ...
      try {
        ...
        if (error.response.status == 403) {
          ...
          setErrors({ insert: "Error occured during link insertion!" });
        }
      }
    }
  };
  ...
  return (
    <div>
      <h2 className="mb-8">Price tracking module</h2>
              ...
              {elements?.map((element, key) => {
                return (
                  ...
                  >
                    ...
                    <td className="py-2 px-4">{element.currentPrice}</td>
                    <td className="py-2 px-4">
                      {new Date(element.lastPriceChange).toLocaleString()}
                    </td>
